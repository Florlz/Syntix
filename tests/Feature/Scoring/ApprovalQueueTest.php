    ...
    public function test_match_queue_resolves_named_sides_and_respects_sport_filters(): void
    {
        ...
        $this->actingAs($creator)
            ...
            ->assertInertia(fn ($page) => $page
                ...
                ->where('workspace.sport.division_count', 1)
                ->where('workspace.sport.entry_count', 2)
                ->where('workspace.sport.player_count', 0)
                ->has('workspace.divisions', 1)
                ->where('workspace.divisions.0.id', (string) $division->getKey())
                ->where('workspace.divisions.0.entry_count', 2)
                ->where('workspace.divisions.0.unlocked_entry_count', 2)
                ->where('workspace.divisions.0.results_state', 'pending_review')
                ->where('result_submissions.0.id', (string) $submission->getKey())
