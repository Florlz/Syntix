...
function NotificationBell({ notifications }) {
    ...
    return <Popover className="relative">
        <PopoverButton
            ...
            className="relative grid size-10 place-items-center rounded-lg text-primary focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent"
        >
        ...
        <PopoverPanel anchor="bottom end" className="z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-border bg-surface shadow-xl">
            <div className="flex items-start justify-between gap-4 border-b border-border px-4 py-3">
                ...
                {unreadCount > 0 ? <button type="button" onClick={markAllRead} className="rounded-md px-2 py-1 text-xs font-semibold text-primary hover:bg-surface-muted focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent">Mark all read</button> : null}
            </div>
            <div className="max-h-96 overflow-y-auto p-2">
                {recent.length > 0 ? recent.map((notification) => {
                    ...
                    return <article key={notification.id} className={`rounded-lg px-3 py-3 ${notification.read_at ? '' : 'bg-primary/5'}`}>
                        <button type="button" onClick={() => openNotification(notification)} className="block w-full text-left focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent">
                        ...
                        {actionHref ? <button type="button" onClick={() => openNotification(notification)} className="mt-3 rounded-md px-2 py-1 text-xs font-semibold text-primary hover:bg-surface-muted focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent">{notification.action.label} <span aria-hidden="true">→</span></button> : null}
                    </article>;
                }) : <p className="px-3 py-8 text-center text-sm text-muted">You're all caught up.</p>}
            </div>
            <div className="border-t border-border px-4 py-3">
                <Link href={route('settings.edit', { section: 'notifications' })} className="text-xs font-semibold text-primary hover:text-primary-hover focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent">Notification settings</Link>
            </div>
        </PopoverPanel>
    </Popover>;
}
...
function Sidebar({ onNavigate, activeSection = null }) {
    ...
    return <div className="flex h-full flex-col bg-sidebar text-white">
        <div className="border-b border-white/10 px-5 py-5"><Link href={route('dashboard')} onClick={onNavigate} className="flex items-center gap-3 rounded-md focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent"><ApplicationLogo className="size-9 text-accent"/><span><strong className="block font-serif text-lg tracking-[0.08em]">SYNTIX</strong><span className="text-[0.62rem] uppercase tracking-[0.2em] text-white/40">Event operations</span></span></Link></div>
        ...
        <nav aria-label="Administration" className="flex-1 overflow-y-auto py-4"><NavItem href={route('dashboard', eventId ? { event: eventId } : {})} icon="overview" label="Overview" active={dashboard} onNavigate={onNavigate}/>{eventId && admin ? <><NavItem href={route('admin.sports.index', eventId)} icon="trophy" label="Sports Directory" active={sportsActive} onNavigate={onNavigate}/><NavItem href={route('admin.departments.index', eventId)} icon="users" label="Departments" active={departmentsActive} onNavigate={onNavigate}/><NavItem href={route('admin.staff.index', eventId)} icon="badge" label="Judges & Tabulators" active={staffActive} badge={badges.staff} onNavigate={onNavigate}/><NavItem href={route('admin.approvals.index', eventId)} icon="clipboard-check" label="Results" active={resultsActive} badge={badges.results} onNavigate={onNavigate}/></> : null}{roles.includes('judge') ? <NavItem href={route('judge.index', eventId ? { event: eventId } : {})} icon="clipboard-check" label="My Judging" active={judgeActive} onNavigate={onNavigate}/> : null}{roles.includes('tabulator') ? <NavItem href={route('tabulator.index', eventId ? { event: eventId } : {})} icon="scoreboard" label="My Tabulation" active={tabulatorActive} onNavigate={onNavigate}/> : null}</nav>
...
export default function AuthenticatedLayout({ header, children, activeSection = null }) {
    ...
    return <div className="min-h-screen bg-background text-foreground">
        <a href="#main-content" className="sr-only z-[60] rounded-sm bg-surface px-4 py-2 text-foreground focus:not-sr-only focus:fixed focus:left-3 focus:top-3">Skip to content</a>
        <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 lg:block"><Sidebar activeSection={activeSection}/></aside>
        <header className="sticky top-0 z-30 border-b border-border bg-surface/95 backdrop-blur lg:ml-64"><div className="flex min-h-16 items-center gap-4 px-4 sm:px-6 lg:px-8"><button type="button" onClick={() => setOpen(true)} aria-label="Open navigation" aria-expanded={open} className="grid size-10 place-items-center rounded-lg text-primary focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent lg:hidden"><AppIcon name="menu"/></button><div className="min-w-0 flex-1 py-3">{header}</div>{auth.global_admin && notifications ? <NotificationBell notifications={notifications} /> : null}</div></header>
