...
export default function Scorecard({ scorecard }) {
    ...
    return (
        <AuthenticatedLayout header={<div><p className="text-[0.68rem] font-bold uppercase tracking-[0.16em] text-primary">My judging</p><h1 className="font-serif text-2xl font-bold text-foreground">{scorecard.competition}</h1></div>}>
            ...
            <main className="min-h-[calc(100vh-4rem)] bg-background px-4 pb-36 pt-5 text-foreground sm:px-7 lg:px-8">
                <form onSubmit={save} className="mx-auto max-w-6xl space-y-5">
                        ...
                        <div className="flex flex-col gap-5 px-5 py-5 sm:px-7 lg:flex-row lg:items-end lg:justify-between">
                            <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                    <p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">{scorecard.competition} / {scorecard.division}</p>
                                ...
                                <h2 className="mt-3 font-serif text-3xl font-bold leading-none text-foreground">{scorecard.entry}</h2>
                                <p className="mt-2 text-sm text-muted">{scorecard.delegation} · {scorecard.contest}</p>
                            </div>
                            <dl className="grid gap-4 text-sm sm:grid-cols-3 lg:min-w-[32rem]">
                                <div><dt className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Venue</dt><dd className="mt-1 font-semibold text-foreground">{scorecard.schedule.venue ?? 'Not scheduled'}</dd></div>
                                <div><dt className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Call time</dt><dd className="mt-1 font-semibold text-foreground">{formatSchedule(scorecard.schedule)}</dd></div>
                                <div><dt className="text-xs font-bold uppercase tracking-[0.12em] text-muted">Assignment</dt><dd className="mt-1 font-semibold text-foreground">Entry {scorecard.navigation.position ?? '—'} of {scorecard.navigation.total}</dd></div>
                            </dl>
                        </div>
                        <nav aria-label="Assigned entries" className="flex items-center justify-between border-t border-border px-5 py-3 sm:px-7">
                    ...
                    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
                        <section aria-labelledby="rubric-heading" className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
                            <div className="flex flex-wrap items-end justify-between gap-3 border-b border-border px-5 py-5 sm:px-6">
                                <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Official rubric</p><h3 id="rubric-heading" className="mt-1 font-serif text-2xl font-bold">Score this entry</h3></div>
                                <p className="text-sm text-muted">Drafts may be saved one criterion at a time.</p>
                            </div>
                            <div className="divide-y divide-border">
                                {scorecard.criteria.map((criterion, index) => {
                                    ...
                                    return (
                                        <fieldset key={criterion.id} disabled={!canEdit || busy} className="grid gap-5 p-5 sm:grid-cols-[minmax(0,1fr)_11rem] sm:items-start sm:p-6">
                        ...
                        <aside className="space-y-5">
                            <section aria-labelledby="source-heading" className="rounded-xl border border-border bg-surface">
                                <div className="px-5 py-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Scoring authority</p><h3 id="source-heading" className="mt-1 font-serif text-xl font-bold">Approved proposal {scorecard.source.pages.length ? `pp. ${scorecard.source.pages.join('–')}` : ''}</h3><p className="mt-2 text-sm text-muted">{scorecard.source.reference ?? 'Approved event scoring source'}</p></div>
                                <div className="border-t border-border px-5 py-3"><span className="rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-primary">{scorecard.source.reliability}</span></div>
                            ...
                            <section aria-labelledby="adjustments-heading" className="rounded-xl border border-border bg-surface p-5">
                                <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Official adjustments</p><h3 id="adjustments-heading" className="mt-1 font-serif text-xl font-bold">Tabulator record</h3></div><strong className="text-xl tabular-nums text-danger">−{Number(scorecard.official_deduction_total).toFixed(2)}</strong></div>
