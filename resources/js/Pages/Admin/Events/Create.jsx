...
export default function Create({ active_users: activeUsers = [] }) {
    const form = useForm({ name: '', slug: '', first_admin_id: '' });
        ...
        <AuthenticatedLayout header={<h1 className="text-2xl font-semibold tracking-tight text-slate-900">Create event shell</h1>}>
            ...
            <main className="min-h-[calc(100vh-9rem)] bg-[#f4f6f8] px-4 py-8 sm:px-6 lg:px-10">
                <form onSubmit={submit} className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                    ...
                    <p className="mt-3 text-sm leading-6 text-slate-500">Creation and first-Admin assignment are recorded as separate audited actions. This form does not open public registration.</p>
                    <div className="mt-8 space-y-5">
                        ...
                        <label className="block">
                            <span className="text-sm font-medium text-slate-700">First event Admin</span>
                            <select value={form.data.first_admin_id} onChange={(event) => form.setData('first_admin_id', event.target.value)} className="mt-2 w-full rounded-xl border-slate-300 focus:border-sky-600 focus:ring-sky-600">
                                <option value="">Grant separately after creation</option>
