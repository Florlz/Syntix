    ...
    return <AuthenticatedLayout header={<div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">{event.name}</p><h1 className="font-serif text-2xl font-bold">Invite event staff</h1></div>}>
        ...
        <main className="min-h-[calc(100vh-4rem)] bg-background px-4 py-8 text-foreground sm:px-6 lg:px-10">
            <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
                ...
                <aside className="rounded-2xl border border-border bg-surface p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Secure handoff</p>{setupUrl ? <div className="mt-4"><h2 className="font-serif text-xl font-bold">Setup invitation ready</h2><p className="mt-2 text-sm leading-6 text-muted">Copy the private link or print one card and hand it directly to the named staff member.</p><label className="mt-4 block"><span className="sr-only">One-time setup link</span><input readOnly value={setupUrl} className="w-full rounded-lg border-border bg-surface-muted text-xs text-foreground"/></label><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-1"><button type="button" onClick={copySetupLink} className="min-h-11 rounded-lg bg-primary px-4 text-sm font-bold text-primary-foreground">{copyState === 'copied' ? 'Copied' : copyState === 'error' ? 'Copy failed' : 'Copy setup link'}</button><button type="button" disabled={qrState !== 'ready'} onClick={() => window.print()} className="min-h-11 rounded-lg border border-border px-4 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-50">{printLabel}</button></div><p aria-live="polite" className="mt-2 text-xs text-muted">{copyState === 'copied' ? 'Setup link copied.' : copyState === 'error' ? 'Copy failed. Select the private link and copy it manually.' : ''}</p><p className="mt-4 rounded-lg bg-danger-surface p-3 text-xs leading-5 text-danger">The link and QR are bearer credentials until used or expired. Do not post, photograph, or leave the card unattended.</p></div> : <div className="mt-4 rounded-xl border border-dashed border-border bg-surface-muted p-5"><h2 className="font-serif text-lg font-bold">No active setup link</h2><p className="mt-2 text-sm leading-6 text-muted">Create an invitation to reveal its one-time link and printable handoff card.</p></div>}</aside>
            </div>
