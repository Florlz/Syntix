...
export default function RegistrationIndex({ event, filters, selection = {}, delegations = [], competitions = [], participants = [], entries = [], summary, options }) {
    ...
    return (
        <AuthenticatedLayout header={<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#9a6e00]">Global Admin · private operations</p><h1 className="mt-1 font-serif text-2xl font-bold text-[#17212b]">Registration Desk</h1></div><Link href={route('dashboard', { event: event.id })} className={buttonSecondary}>Back to dashboard</Link></div>}>
            <Head title={`${event.name} Registration Desk`} />
