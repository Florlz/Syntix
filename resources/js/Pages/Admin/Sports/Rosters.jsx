    ...
    return <div className="flex flex-col gap-5">
        <section className={`${surface} rounded-xl p-5 sm:p-6`} aria-labelledby="roster-readiness-title">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Roster readiness</p><h3 id="roster-readiness-title" className="mt-1 font-serif text-2xl font-bold text-foreground">{isLocked ? 'Approved team sheet' : rosterReady ? 'Team sheet is ready for approval' : `${readinessBlockers.length || 1} item${readinessBlockers.length === 1 ? '' : 's'} to finish`}</h3><p className="mt-2 text-sm text-muted">{isLocked ? `Approved revision ${entry.approval_revision || '—'}. Normal roster editing is hidden while this approved snapshot is active.` : `${playerCount}${playerMaximum === null || playerMaximum === undefined ? '' : ` / ${playerMaximum}`} players${rosterReady ? ' · All roster requirements are complete.' : ''}`}</p></div>
            ...
            {archived ? <p className="mt-4 text-sm text-muted">Archived events are read-only.</p> : entryCapabilities.published && !isLocked ? <div className="mt-4 flex flex-col gap-3 rounded-lg border border-accent/40 bg-accent/10 p-4 text-sm text-muted sm:flex-row sm:items-center sm:justify-between"><p><strong className="text-foreground">Roster approved by the published bracket.</strong> The approved team sheet can no longer be edited.</p><Link href={route('admin.sports.tournament', [event.id, division.id])} className={quiet}>View bracket<AppIcon name="arrow-right" className="size-4" /></Link></div> : null}
