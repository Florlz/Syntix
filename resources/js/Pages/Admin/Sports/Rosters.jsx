...
function departmentStatus(department) {
    if (department.state === 'locked') return 'Locked';
    if (department.state === 'ready') return 'Ready';
...
function RosterDetail({ event, sport, division, selected, options, archived, departmentName = null }) {
    ...
    if (!entry) {
        ...
        return <section className={`${surface} max-w-3xl rounded-xl p-5 shadow-xs sm:p-7`}><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Start team sheet</p><h3 className="mt-2 font-serif text-2xl font-bold text-foreground">{selected?.department_id ? `Start the ${rosterLabel} roster` : 'Select a team'}</h3><p className="mt-2 max-w-xl text-sm leading-6 text-muted">{selected?.department_id ? `Create an empty team sheet for ${departmentName || 'this team'}, then add its players.` : 'Choose a team before starting its team sheet.'}</p>{selected?.department_id ? <form className="mt-5" onSubmit={(eventObject) => { eventObject.preventDefault(); router.post(route('admin.department-rosters.store', [event.id, division.id, selected.department_id]), {}, { preserveScroll: true }); }}><button className={`${primary} w-full sm:w-auto`} disabled={archived}>Start team sheet</button></form> : null}</section>;
    }

    return <div className="flex flex-col gap-5">
        <section className={`${surface} rounded-xl p-5 sm:p-6`} aria-labelledby="roster-readiness-title">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Roster readiness</p><h3 id="roster-readiness-title" className="mt-1 font-serif text-2xl font-bold text-foreground">{isLocked ? 'Approved team sheet' : rosterReady ? 'Team sheet is ready to lock' : `${readinessBlockers.length || 1} item${readinessBlockers.length === 1 ? '' : 's'} to finish`}</h3><p className="mt-2 text-sm text-muted">{isLocked ? `Locked revision ${entry.approval_revision || '—'}. Normal roster editing is hidden while this approved snapshot is active.` : `${playerCount}${playerMaximum === null || playerMaximum === undefined ? '' : ` / ${playerMaximum}`} players${rosterReady ? ' · All roster requirements are complete.' : ''}`}</p></div>
                <span className={`inline-flex w-fit items-center rounded-full border px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] ${isLocked ? 'border-primary/30 bg-primary/10 text-primary' : rosterReady ? 'border-primary/30 bg-primary/10 text-primary' : 'border-accent/40 bg-accent/10 text-accent-foreground'}`}>{isLocked ? 'Locked' : rosterReady ? 'Ready to lock' : 'Needs attention'}</span>
            </div>
            {!isLocked && readinessBlockers.length ? <ul className="mt-4 flex flex-col gap-2 text-sm text-danger">{readinessBlockers.map((blocker) => <li key={blocker} className="border-l-2 border-danger pl-3">{blocker}</li>)}</ul> : null}
            ...
            {archived ? <p className="mt-4 text-sm text-muted">Archived events are read-only.</p> : entryCapabilities.published && !isLocked ? <div className="mt-4 flex flex-col gap-3 rounded-lg border border-accent/40 bg-accent/10 p-4 text-sm text-muted sm:flex-row sm:items-center sm:justify-between"><p><strong className="text-foreground">Roster locked by published bracket.</strong> The approved team sheet can no longer be edited.</p><Link href={route('admin.sports.tournament', [event.id, division.id])} className={quiet}>View bracket<AppIcon name="arrow-right" className="size-4" /></Link></div> : null}
            ...
            <div className="mt-5 flex flex-wrap gap-2">
                {!isLocked && !archived && !entryCapabilities.published && canReview ? <button type="button" className={primary} onClick={() => setLockReviewOpen(true)} disabled={statusForm.processing}>Lock team sheet</button> : null}
                {isLocked && !archived && entryCapabilities.can_reopen ? <button type="button" className={quiet} onClick={() => setReopenOpen(true)}>Reopen team sheet</button> : null}
        ...
        />
        <details className="overflow-hidden border-y border-[#CFD6D3] bg-white"><summary className="cursor-pointer list-none px-5 py-4 text-sm font-bold text-[#17212B] focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#D5A21F]">Team staff (optional) - {coaches.length}</summary><div className="border-t border-[#E6EAE8] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-[#68767E]">Student and faculty coaches connected to this team.</p><div className="flex flex-wrap gap-2"><button type="button" className={primary} onClick={() => setCoachSupportOpen(true)} disabled={archived || entryCapabilities.published}>Add coach or staff</button><Link href={`${route('admin.departments.show', [event.id, selected.department_id])}?view=coaches`} className={quiet}>Manage all team staff</Link></div></div>{coaches.length ? <div className="mt-4 divide-y divide-[#E6EAE8] border-y border-[#E6EAE8]">{coaches.map((coach) => <div key={`${coach.id}-${coach.coach_type}-${coach.scope_key}`} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><strong className="text-sm">{coach.display_name}</strong><p className="mt-1 text-xs text-[#68767E]">{coach.title} / {coach.coach_type.replaceAll('_', ' ')}</p></div><span className="text-xs font-semibold text-[#0B536D]">{coach.scope_type === 'programme_family' ? 'Shared assignment' : 'This division'}</span></div>)}</div> : <p className="mt-4 text-sm text-[#68767E]">No team staff are assigned to this roster yet. Add the first coach or staff person when ready.</p>}</div></details>
        <details className="overflow-hidden border-y border-[#CFD6D3] bg-white"><summary className="cursor-pointer list-none px-5 py-4 text-sm font-bold text-[#17212B] focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#D5A21F]">Roster history - {history.length} inactive</summary><div className="border-t border-[#CFD6D3]"><RosterPlayerList title="Inactive players" description="Previous roster places remain available for review and restoration." members={history} onManage={setProfile} emptyMessage="No inactive roster history." /></div></details>
        <SlideOver show={lockReviewOpen} title="Lock team sheet" onClose={() => setLockReviewOpen(false)}><form onSubmit={lock} className="space-y-5"><div><h3 className="font-serif text-2xl font-bold text-foreground">Lock {departmentName || 'team'} team sheet?</h3><p className="mt-2 text-sm leading-6 text-muted">This creates the approved competition roster for {sport.name} {division.name}. {players.length} players will be included. After locking, roster changes require reopening the team sheet with a correction reason.</p><dl className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-lg border border-border bg-surface p-3"><dt className="text-xs uppercase text-muted">Players</dt><dd className="mt-1 text-xl font-bold text-foreground">{players.length}</dd></div><div className="rounded-lg border border-border bg-surface p-3"><dt className="text-xs uppercase text-muted">Team staff</dt><dd className="mt-1 text-xl font-bold text-foreground">{coaches.length}</dd></div></dl></div>{readinessNotices.length ? <section className="border-l-2 border-accent pl-4"><h3 className="text-sm font-bold text-foreground">Good to know</h3><ul className="mt-2 flex flex-col gap-1 text-sm text-muted">{readinessNotices.map((notice) => <li key={notice}>{notice}</li>)}</ul></section> : null}<label className="flex items-start gap-3 rounded-lg border border-accent/40 bg-accent/10 p-4 text-sm font-semibold text-foreground"><input required type="checkbox" checked={statusForm.data.roster_review_confirmed} onChange={(eventObject) => statusForm.setData('roster_review_confirmed', eventObject.target.checked)} className="mt-0.5 rounded-sm border-border text-primary focus:ring-accent" />I confirm that the roster details and required external documents were reviewed.</label>{statusErrors.length ? <ul role="alert" className="list-disc space-y-1 pl-5 text-sm font-semibold text-danger">{statusErrors.map((error) => <li key={error}>{error}</li>)}</ul> : null}<button className={primary} disabled={!statusForm.data.roster_review_confirmed || statusForm.processing}>{statusForm.processing ? 'Locking team sheet...' : 'Lock team sheet'}</button></form></SlideOver>
        ...
        <SlideOver show={reopenOpen} title="Reopen team sheet" onClose={() => setReopenOpen(false)}><form onSubmit={reopen} className="space-y-4"><p className="text-sm leading-6 text-muted">The approved roster snapshot remains in history. Reopening allows corrections and requires a reason.</p><label className="block text-sm font-bold text-foreground">Reason<textarea required rows="4" value={statusForm.data.reason} onChange={(eventObject) => statusForm.setData('reason', eventObject.target.value)} className={input} /></label>{statusErrors.length ? <ul role="alert" className="list-disc space-y-1 pl-5 text-sm font-semibold text-danger">{statusErrors.map((error) => <li key={error}>{error}</li>)}</ul> : null}<button className={primary} disabled={statusForm.processing}>{statusForm.processing ? 'Reopening team sheet...' : 'Reopen team sheet'}</button></form></SlideOver>
...
export default function Rosters({ event, sport, division, selectedDepartment, workspace, options, archived, departmentName = null }) {
    ...
    if (!division) return <section className={`${surface} rounded-xl p-6`}><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Teams &amp; Rosters</p><h2 className="mt-2 font-serif text-2xl font-bold text-foreground">Choose a division</h2><p className="mt-2 text-sm text-muted">Pick a division above to see its teams and team sheets.</p></section>;
    ...
    return <div className="flex flex-col gap-5">
        <header className="flex flex-col gap-3 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Teams &amp; Rosters</p><h2 className="mt-1 font-serif text-3xl font-bold text-foreground">{division.name}</h2><p className="mt-1 text-sm text-muted">Manage the teams competing in {sport.name} {division.name}.</p></div><p className="text-sm font-semibold text-muted">{departments.length} teams · {readyCount} ready · {draftCount} draft · {attentionCount} need attention</p></header>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <label className="text-sm font-bold text-foreground">Team status<select aria-label="Team status" value={statusFilter} onChange={(eventObject) => setStatusFilter(eventObject.target.value)} className={input}><option value="all">All statuses</option><option value="needs_attention">Needs attention</option><option value="draft">Draft</option><option value="not_started">Not started</option><option value="ready">Ready</option><option value="locked">Locked</option></select></label>
        </div>
