...
function friendlyBlocker(value) {
    ...
    if (/No Entries have been registered/i.test(text)) return 'No teams have been registered for this division.';
    if (/Lock at least one eligible Entry/i.test(text)) return 'Complete and lock at least one team sheet before generating the draw.';
    ...
    return <AuthenticatedLayout header={<div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">{event.name}</p><h1 className="font-serif text-2xl font-bold">Bracket</h1></div>}>
        ...
        <main className="bg-background p-4 sm:p-7 lg:p-8"><SportWorkspaceShell event={event} sport={shellSport} division={division} divisions={divisions} activeSection="bracket">
            <div className="flex flex-col gap-5">
                ...
                <header className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">{discipline ? 'Discipline bracket' : 'Division bracket'}</p><h2 className="mt-1 font-serif text-3xl font-bold text-foreground sm:text-4xl">Bracket</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Create and publish the official draw for {heading}.</p></div><span className="inline-flex min-h-8 items-center rounded-full border border-border bg-surface-muted px-3 text-xs font-bold uppercase tracking-[0.12em] text-muted">{status}</span></header>
