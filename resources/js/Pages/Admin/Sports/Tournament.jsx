...
function MatchCard({ node, nodeRef }) {
    ...
    return <article ref={nodeRef} data-node-id={node.id} className="relative z-10 w-[13.5rem] border border-border bg-surface text-foreground ">
...
function UncontestedSurface({ event, sport, division, entry }) {
    ...
    return <section className={`${panel} overflow-hidden`} aria-labelledby="uncontested-title">
        <div className="border-b border-primary/20 bg-primary/10 p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <h3 id="uncontested-title" className="mt-1 text-2xl font-bold text-foreground">One eligible team advances</h3>
        ...
        <div className="p-5 sm:p-6">
            ...
            <div className="mt-3 flex flex-col gap-2 rounded-sm border border-border bg-surface-muted p-4 sm:flex-row sm:items-center sm:justify-between">
                <div><p className="text-xl font-bold text-foreground">{teamName}</p>{teamCode ? <p className="mt-1 text-sm font-semibold text-muted">{teamCode}</p> : null}</div>
...
export default function Tournament({ event, sport, sports = [], division, discipline, proposal, entries = [], tournament, bracket, blockers = [], can_generate: canGenerate, can_redraw: canRedraw, can_publish: canPublish, is_archived: isArchived }) {
    ...
    return <AuthenticatedLayout header={<div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">{event.name}</p><h1 className="text-2xl font-bold">Bracket</h1></div>}>
        ...
        <main className={adminStyles.page}><SportWorkspaceShell event={event} sport={shellSport} division={division} divisions={divisions} activeSection="bracket">
            <div className="flex flex-col gap-5">
                {flash?.status ? <div role="status" className="rounded-sm border border-primary bg-primary/10 p-4 text-sm font-semibold text-primary">{flash.status}</div> : null}
                {Object.keys(errors).length ? <div role="alert" className="rounded-sm border border-danger bg-danger-surface p-4 text-sm font-semibold text-danger">{Object.values(errors).flat().join(' ')}</div> : null}

                <header className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">{discipline ? 'Event bracket' : 'Division bracket'}</p><h2 className="mt-1 text-3xl font-bold text-foreground sm:text-4xl">Bracket</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Create and publish the official draw for {heading}.</p></div><span className="inline-flex min-h-8 items-center rounded-full border border-border bg-surface-muted px-3 text-xs font-bold uppercase tracking-[0.12em] text-muted">{status}</span></header>

                <div className="grid gap-3 sm:grid-cols-3"><div className={`${panel} p-4`}><p className="text-xs font-bold uppercase tracking-[0.14em] text-muted">Competition format</p><p className="mt-2 text-xl font-bold text-foreground">{formatLabel(format)}</p><p className="mt-1 text-xs text-muted">{discipline?.family ?? division.participant_mode ?? 'Rules not configured'}</p></div><div className={`${panel} p-4`}><p className="text-xs font-bold uppercase tracking-[0.14em] text-muted">{discipline ? 'Team sheets and lineups' : 'Team sheets'}</p><p className="mt-2 text-xl font-bold text-foreground">{lockedTeams} / {requiredTeams}</p><p className="mt-1 text-xs text-muted">{readinessDetail}</p></div><div className={`${panel} p-4`}><p className="text-xs font-bold uppercase tracking-[0.14em] text-muted">Teams in the draw</p><p className="mt-2 text-xl font-bold text-foreground">{eligibleCount}</p><p className="mt-1 text-xs text-muted">included in this draw</p></div></div>
