...
function ErrorText({ value }) { return value ? <p className="mt-1 text-xs font-semibold text-danger">{value}</p> : null; }
...
export default function RosterAddPlayers({ event, entry, departmentId, participants = [], roles = [], archived, onClose }) {
    ...
    const createPlayer = (eventObject) => {
        eventObject.preventDefault();
        quickForm.post(route('admin.participants.store', event.id), {
            preserveScroll: true,
            onSuccess: (page) => {
                const createdIds = page?.props?.flash?.selected_participant_ids || page?.props?.selected_participant_ids || [];
                setSelected((current) => new Set([...current, ...createdIds.map(String)]));
    ...
    return <div className="space-y-5">
        <div className="flex flex-wrap items-end justify-between gap-3 border-b border-border pb-4"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Add players</p><h3 className="mt-1 text-2xl font-bold">Build this team sheet</h3><p className="mt-1 text-sm text-muted">Active players on the team sheet are cleared to compete when the roster is approved.</p></div><button type="button" className={quiet} onClick={onClose}>Done</button></div>
        <label className="block"><span className="text-sm font-bold">Search department players</span><div className="relative"><AppIcon name="search" className="pointer-events-none absolute left-3 top-3 size-4 text-muted" /><input value={query} onChange={(eventObject) => setQuery(eventObject.target.value)} className={`${input} pl-9`} placeholder="Name or student number" /></div></label>
        <div className="divide-y divide-border border-y border-border">{available.length ? available.map((person) => <label key={person.id} className="flex items-center gap-3 py-3"><input type="checkbox" value={String(person.id)} checked={selected.has(String(person.id))} onChange={() => toggle(person.id)} className="size-4 rounded-sm border-border text-primary focus:ring-accent" /><span className="min-w-0 flex-1"><strong className="block text-sm">{person.display_name}</strong><span className="block text-xs text-muted">{person.student_number || 'No student number'} · {person.membership ? 'Inactive history · can restore' : 'Not on roster'}</span></span><select aria-label={`Role for ${person.display_name}`} value={roleByParticipant[person.id] || 'student_athlete'} onChange={(eventObject) => setRoleByParticipant((current) => ({ ...current, [person.id]: eventObject.target.value }))} className="rounded-md border-border py-2 text-xs" disabled={!selected.has(String(person.id))}>{roles.filter((role) => playerRoles.has(role.value)).map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select></label>) : <p className="py-8 text-center text-sm text-muted">No unassigned department players match this search.</p>}</div>
        <form onSubmit={addSelected} className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted"><strong className="text-foreground">{selected.size}</strong> selected</p><button type="submit" className={primary} disabled={archived || !selected.size || memberForm.processing}>{memberForm.processing ? 'Adding players…' : 'Add selected to roster'}</button></div><ErrorSummary errors={memberForm.errors} />{addSuccess ? <p role="status" className="text-sm font-semibold text-primary">Players added. The roster is ready for review.</p> : null}</form>
        <div className="grid gap-3 border-t border-border pt-5 sm:grid-cols-2"><button type="button" className={quiet} onClick={() => setShowQuickCreate((value) => !value)} disabled={archived}>Create a new player</button><button type="button" className={quiet} onClick={() => setShowCsv((value) => !value)} disabled={archived}>Import CSV</button></div>
        {showQuickCreate ? <form onSubmit={createPlayer} className="space-y-4 border border-border bg-surface-muted p-4"><p className="text-sm font-bold">New player in this department</p><p className="text-xs leading-5 text-muted">This creates the shared event profile first. You will still add them to this roster separately.</p><label className="block text-sm font-semibold">Display name<input required value={quickForm.data.display_name} onChange={(eventObject) => quickForm.setData('display_name', eventObject.target.value)} className={input} disabled={archived} /></label><div className="grid gap-3 sm:grid-cols-2"><label className="block text-sm font-semibold">Student number<input value={quickForm.data.student_number} onChange={(eventObject) => quickForm.setData('student_number', eventObject.target.value)} className={input} disabled={archived} /></label><label className="block text-sm font-semibold">Email<input type="email" value={quickForm.data.email} onChange={(eventObject) => quickForm.setData('email', eventObject.target.value)} className={input} disabled={archived} /></label></div><button type="submit" className={primary} disabled={archived || quickForm.processing}>Create player profile</button><ErrorText value={quickForm.errors.display_name || quickForm.errors.student_number || quickForm.errors.event_delegation_id} /></form> : null}
        {showCsv ? <section className="space-y-4 border border-border bg-surface-muted p-4"><div><p className="text-sm font-bold">Guided CSV import</p><p className="mt-1 text-xs leading-5 text-muted">Import profiles only. Nothing is added to the roster until you select them above.</p></div><a href="data:text/csv;charset=utf-8,department_code,student_number,display_name,given_name,family_name,email,phone,private_notes,active%0A" download="syntix-players-template.csv" className="text-sm font-bold text-primary underline">Download template</a><input type="file" accept=".csv,text/csv" onChange={(eventObject) => { setCsvFile(eventObject.target.files?.[0] || null); setCsvResult(null); }} className="block w-full text-sm" /><div className="flex flex-wrap gap-2"><button type="button" className={quiet} onClick={inspectCsv} disabled={!csvFile}>Inspect file</button><button type="button" className={primary} onClick={confirmCsv} disabled={!csvFile || !csvResult || csvResult.errors?.length}>Confirm import</button></div>{csvResult ? <div className="space-y-3 text-sm"><p className="font-semibold">{csvResult.errors?.length ? `${csvResult.errors.length} issue${csvResult.errors.length === 1 ? '' : 's'} found` : `${csvResult.rows?.length || 0} rows ready`}</p>{sourceHeaders.length ? <div className="grid gap-2 sm:grid-cols-2">{Object.keys(csvResult.mapping || {}).map((target) => <label key={target} className="text-xs font-semibold capitalize">{target.replaceAll('_', ' ')}<select value={csvMapping[target] || ''} onChange={(eventObject) => setCsvMapping((current) => ({ ...current, [target]: eventObject.target.value }))} className={input}><option value="">Not mapped</option>{sourceHeaders.map((header) => <option key={header} value={header}>{header}</option>)}</select></label>)}</div> : null}{csvResult.errors?.length ? <ul className="list-disc space-y-1 pl-5 text-xs text-danger">{csvResult.errors.slice(0, 8).map((error, index) => <li key={index}>{error.message || error}</li>)}</ul> : null}</div> : null}</section> : null}
    </div>;
}
