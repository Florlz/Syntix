...
export default function RosterAddPlayers({ event, entry, departmentId, participants = [], roles = [], archived, onClose }) {
    ...
    return <div className="space-y-5">
        ...
        <div className="divide-y divide-[#E6EAE8] border-y border-[#CFD6D3]">{available.length ? available.map((person) => <label key={person.id} className="flex items-center gap-3 py-3"><input type="checkbox" value={String(person.id)} checked={selected.has(String(person.id))} onChange={() => toggle(person.id)} className="size-4 rounded-sm border-[#B8C3C0] text-[#0B536D] focus:ring-[#D5A21F]" /><span className="min-w-0 flex-1"><strong className="block text-sm">{person.display_name}</strong><span className="block text-xs text-[#68767E]">{person.student_number || 'No student number'} · {person.membership ? 'Inactive history · can restore' : 'Not on roster'}</span></span><select aria-label={`Role for ${person.display_name}`} value={roleByParticipant[person.id] || 'student_athlete'} onChange={(eventObject) => setRoleByParticipant((current) => ({ ...current, [person.id]: eventObject.target.value }))} className="rounded-md border-[#B8C3C0] py-2 text-xs" disabled={!selected.has(String(person.id))}>{roles.filter((role) => playerRoles.has(role.value)).map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select></label>) : <p className="py-8 text-center text-sm text-[#68767E]">No unassigned department players match this search.</p>}</div>
