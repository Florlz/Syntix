...
export default function AssignmentsSection({ staff = [], event, onManage }) {
    ...
    const judgingPanelCount = useMemo(
        () => new Set(rows.filter((row) => row.role === 'Judge').map((row) => row.panel?.contest_id).filter(Boolean)).size,
        [rows],
    );

    return <section aria-labelledby="assignments-heading" className="space-y-5">
        <header><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Assignments</p><h2 id="assignments-heading" className="mt-1 text-3xl font-bold">Assignment coverage</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">See which Judges are assigned to judging panels and where Tabulators have operational responsibility.</p></header>
        {event.archived ? <div className="rounded-sm border border-accent bg-accent/10 p-4 text-sm text-foreground"><strong>Archived event.</strong> Assignment history remains available, but this event can no longer be modified.</div> : null}
        ...
        {missing.length ? <section aria-labelledby="needs-coverage-heading" className="rounded-sm border border-accent bg-accent/10 p-5"><h3 id="needs-coverage-heading" className="text-xl font-bold">Needs coverage</h3><div className="mt-3 grid gap-3 md:grid-cols-2">{missing.map(({ person, role: missingRole }) => <button type="button" key={`${person.id}-${missingRole}`} onClick={() => onManage(person)} className="rounded-sm border border-accent/50 bg-surface p-4 text-left focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent"><p className="font-bold">{person.name}</p><p className="mt-1 text-xs font-bold uppercase tracking-[0.1em] text-primary">{missingRole}</p><p className="mt-2 text-sm text-muted">{missingRole === 'judge' ? 'Not assigned to a judging panel' : 'No Division or Contest assignment'}</p></button>)}</div></section> : null}
        {rows.length ? <div className="overflow-hidden rounded-sm border border-border bg-surface"><div className="hidden grid-cols-[minmax(10rem,1fr)_8rem_minmax(14rem,1.5fr)_10rem_auto] gap-4 border-b border-border bg-surface-muted px-5 py-3 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-muted md:grid"><span>Person</span><span>Role</span><span>Target</span><span>Scope</span><span className="sr-only">Action</span></div>{rows.map((row) => <div key={row.id} className="grid gap-3 border-b border-border px-5 py-4 last:border-b-0 md:grid-cols-[minmax(10rem,1fr)_8rem_minmax(14rem,1.5fr)_10rem_auto] md:items-center md:gap-4"><div><p className="font-bold">{row.person.name}</p><p className="text-xs text-muted md:hidden">{row.person.email}</p></div><span className="w-fit rounded-full bg-accent/15 px-2 py-1 text-[0.65rem] font-bold uppercase text-accent-foreground">{row.role}</span><div><p className="text-sm">{row.target}</p><p className="mt-1 text-xs text-muted">{row.detail}</p></div><p className="text-xs font-bold uppercase tracking-[0.1em] text-muted">{row.scope === 'judging_panel' ? 'Judging panel' : row.scope}</p>{row.role === 'Judge' ? <a href={route('admin.staff.index', { event: event.id, section: 'readiness', focus: row.panel?.division_id })} className="text-sm font-bold text-primary">Manage panel</a> : <button type="button" onClick={() => onManage(row.person)} className="text-left text-sm font-bold text-primary">Manage</button>}</div>)}</div> : <div className="rounded-sm border border-dashed border-border bg-surface p-10 text-center text-sm text-muted">No assignment matches these filters.</div>}
    </section>;
}
