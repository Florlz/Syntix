    ...
    return <Modal show maxWidth="lg" onClose={onClose}>
        <form onSubmit={onSubmit} className="border border-border bg-surface p-5 text-foreground">
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-2">
                <button type="button" onClick={onClose} className="min-h-10 rounded-sm border border-border px-4 text-sm font-bold text-foreground">Cancel</button>
                <button type="submit" disabled={form.processing} className="min-h-10 rounded-sm bg-danger px-4 text-sm font-bold text-white disabled:opacity-50">{form.processing ? 'Processing…' : request.submitLabel}</button>
            </div>
        </form>
    </Modal>;
}
            ...
            <div className="space-y-5">
                ...
                <section className="rounded-sm border border-border bg-surface p-5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        {person.account_state === 'active' ? <button type="button" disabled={event.archived} onClick={() => openReason({ title: 'Disable account', description: `Disable ${person.name}'s account and revoke active sessions.`, submitLabel: 'Disable account', url: route('admin.staff.disable', [event.id, person.id]), successMessage: 'Account disabled.' })} className="inline-flex min-h-10 items-center justify-center rounded-sm bg-danger px-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">Disable account</button> : <button type="button" disabled={event.archived || account.processing} onClick={() => account.patch(route('admin.staff.enable', [event.id, person.id]), { preserveScroll: true, onSuccess: () => setMessage('Account reactivated.') })} className="inline-flex min-h-10 items-center justify-center rounded-sm bg-primary px-4 text-sm font-bold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">Reactivate account</button>}
                    </div>
