...
function ReasonDialog({ request, form, onClose, onSubmit }) {
    ...
    return <Modal show maxWidth="lg" onClose={onClose}>
        <form onSubmit={onSubmit} className="border border-border bg-surface p-5 text-foreground">
            <DialogTitle as="h2" className="text-xl font-bold">{request.title}</DialogTitle>
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-2">
                <button type="button" onClick={onClose} className="min-h-11 rounded-sm border border-control-border px-4 text-sm font-bold text-foreground">Cancel</button>
                <button type="submit" disabled={form.processing} className="min-h-11 rounded-sm bg-danger px-4 text-sm font-bold text-white disabled:opacity-50">{form.processing ? 'Processing…' : request.submitLabel}</button>
            </div>
        </form>
    </Modal>;
}
...
function Feedback({ message, errors }) {
    return <>{message ? <p role="status" className="mt-3 rounded-sm bg-primary/10 p-3 text-sm text-foreground">{message}</p> : null}{errors && Object.keys(errors).length ? <p role="alert" className="mt-3 rounded-sm bg-danger-surface p-3 text-sm text-danger">{Object.values(errors).join(' ')}</p> : null}</>;
}
    ...
    return <>
        <SlideOver show title={`Staff record: ${person.name}`} onClose={() => reasonRequest ? setReasonRequest(null) : onClose()}>
            <div className="space-y-5">
                {event.archived ? <div className="rounded-sm border border-accent bg-accent/10 p-4 text-sm"><strong>Archived event.</strong> This record is read-only.</div> : null}
