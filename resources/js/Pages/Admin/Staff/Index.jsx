...
function SetupInvitationNotice({ event, invitation }) {
    ...
    return <section aria-label="New setup invitation" className="rounded-xl border border-accent bg-accent/10 p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">New setup invitation ready</p><h2 className="mt-1 font-serif text-xl font-bold">Private handoff for {invitation.name}</h2><p className="mt-2 text-sm text-muted">Previous unused setup links have been invalidated.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={copySetupLink} className={action}>{copyState === 'copied' ? 'Copied' : copyState === 'error' ? 'Copy failed' : 'Copy private link'}</button><button type="button" onClick={() => window.print()} className="min-h-10 rounded-lg border border-border bg-surface px-4 text-sm font-bold">Print setup card</button></div></div><p aria-live="polite" className="mt-2 text-xs text-muted">{copyState === 'copied' ? 'Setup link copied.' : copyState === 'error' ? 'Copy failed. Select the private link and copy it manually.' : `Role: ${invitation.role_label}. Expires ${invitation.expires_at ? new Date(invitation.expires_at).toLocaleString() : '24 hours after issue'}.`}</p><label className="mt-4 block"><span className="sr-only">One-time setup link</span><input readOnly value={setupUrl} className="w-full rounded-lg border-border bg-surface text-xs text-foreground"/></label><StaffSetupHandoffCard eventName={event.name} staffName={invitation.name} roleLabel={invitation.role_label} expiresAt={invitation.expires_at} setupUrl={setupUrl}/></section>;
}
