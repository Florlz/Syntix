...
const buttonPrimary = 'inline-flex min-h-11 items-center justify-center rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground transition hover:bg-primary-hover focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50';
const buttonDanger = 'inline-flex min-h-11 items-center justify-center rounded-xl border border-danger bg-danger-surface px-4 py-2.5 text-sm font-bold text-danger transition hover:border-danger focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-danger focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50';
...
function ScopeHeader({ event, scope }) {
    ...
    return (
        <section className="overflow-hidden rounded-3xl bg-sidebar p-6 text-white shadow-xs sm:p-8">
...
function OutcomeReview({ submission }) {
    ...
    return (
        <article className="rounded-2xl border border-border bg-surface p-5 shadow-xs sm:p-6">
...
export default function Index({ event, result_submissions: submissions = [], division_placements: placements = [], scope = {}, workspace = null }) {
    ...
    return (
        <AuthenticatedLayout header={<div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">{scopeLabel || event.name}</p><h1 className="font-serif text-2xl font-bold">Results</h1></div>}>
            ...
            <main className="min-h-[calc(100vh-4rem)] bg-background p-4 sm:p-7 lg:p-8"><ResultsWorkspace event={event} workspace={workspace} scope={scope}><div className="mx-auto flex max-w-6xl flex-col gap-6">
                ...
                <div className="flex flex-wrap gap-2 rounded-2xl border border-border bg-surface p-2 shadow-xs" role="tablist" aria-label="Results queues">
                ...
                {queue === 'needs' ? <>
                    <section className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs"><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Needs review</p><p className="mt-2 font-serif text-3xl font-bold text-[#17212b]">{needsCount}</p><p className="mt-1 text-sm text-slate-500">Items blocking official results.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs"><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Match results</p><p className="mt-2 font-serif text-3xl font-bold text-[#17212b]">{submissions.length}</p><p className="mt-1 text-sm text-slate-500">Confirm score and winner.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs"><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Final standings</p><p className="mt-2 font-serif text-3xl font-bold text-[#17212b]">{placements.length}</p><p className="mt-1 text-sm text-slate-500">Confirm rank and points.</p></div></section>
                ...
                </> : <section className="rounded-2xl border border-slate-200 bg-white p-8 shadow-xs"><h2 className="font-serif text-2xl font-bold text-[#17212b]">Official results</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Approved scores, brackets, and standings are kept in the selected sport’s workspace. Open a sport to review its official history without mixing it with pending work.</p>{officialSportHref ? <Link href={officialSportHref} className={`${buttonPrimary} mt-5 w-fit`}>Open sport results</Link> : null}</section>}
