...
function ScopeHeader({ event, scope }) {
    ...
    if (isScoped) {
        return <header className="flex flex-col gap-2 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Results</p><h2 className="text-3xl font-bold text-foreground">Review submitted outcomes</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Confirm scores first, then approve final standings for this division.</p></div><span className="text-sm font-semibold text-muted">{scope.division || scope.competition}</span></header>;
    }
...
function JudgedEvidence({ payload, selectedScorecards, onSelectionChange }) {
    ...
    return <section aria-labelledby="judged-evidence-heading" className="mt-5 overflow-hidden border border-border">
        <header className="bg-surface-muted p-4"><p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">Judged evidence</p><h4 id="judged-evidence-heading" className="mt-1 text-xl font-bold">Panel totals and final ranking</h4><p className="mt-1 text-sm text-muted">{payload.aggregation_method} aggregation · {payload.source_reference || 'Source reference unavailable'}</p></header>
        <div className="overflow-x-auto"><table className="w-full min-w-[48rem] text-left text-sm"><thead className="border-y border-border bg-surface text-xs uppercase tracking-[0.1em] text-muted"><tr><th className="px-4 py-3">Rank / entry</th>{panel.map((card) => <th key={card.judge_id} className="px-3 py-3">{card.judge || `Judge ${card.judge_id}`}</th>)}<th className="px-3 py-3">Aggregate</th><th className="px-3 py-3">Adjustments</th><th className="px-4 py-3">Final</th></tr></thead><tbody className="divide-y divide-border">{rows.map((row) => <tr key={row.entry_id}><th className="px-4 py-4"><span className="mr-2 font-condensed text-primary">#{row.rank}</span>{row.entry}<span className="block text-xs font-normal text-muted">{row.delegation}</span></th>{row.scorecards.map((card) => <td key={card.scorecard_id} className="px-3 py-4 align-top"><label className="flex items-start gap-2"><input type="checkbox" checked={selectedScorecards.includes(Number(card.scorecard_id))} onChange={() => toggle(Number(card.scorecard_id))} className="mt-0.5 rounded border-border text-primary focus:ring-accent"/><span><span className="block font-condensed font-bold tabular-nums">{card.raw_total}</span><span className="text-[0.65rem] text-muted">rev. {card.revision} · return for correction</span></span></label><details className="mt-2"><summary className="cursor-pointer text-xs font-bold text-primary">Criteria</summary><dl className="mt-1 space-y-1 text-xs text-muted">{(card.criteria || []).map((criterion) => <div key={criterion.criterion_id} className="flex justify-between gap-2"><dt>{criterion.criterion}</dt><dd className="font-mono">{criterion.weighted_value}</dd></div>)}</dl></details></td>)}<td className="px-3 py-4 font-condensed tabular-nums">{row.aggregate_raw_total}</td><td className="px-3 py-4 font-condensed text-danger">−{row.adjustment_total}</td><td className="px-4 py-4 font-condensed font-bold tabular-nums">{row.final_total}</td></tr>)}</tbody></table></div>
        ...
        <article className="border border-border bg-surface p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <h3 className="mt-2 text-xl font-bold text-foreground">{submission.contest || 'Match result'}</h3>
...
function PlacementReview({ placement }) {
    ...
    return (
        <article className="border border-border bg-surface p-5 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div><div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-accent-foreground"><span>{placement.competition}</span><span className="text-muted">/</span><span>{placement.division}</span></div><h3 className="mt-2 text-xl font-bold text-foreground">Final standings</h3><p className="mt-2 text-sm text-muted">Revision {placement.revision} · submitted by {placement.submitted_by || 'Unknown'} · {dateTime(placement.submitted_at)}</p></div>
                <StatusPill>Needs review</StatusPill>
            </div>
            <div className="mt-5 overflow-x-auto rounded-sm border border-border"><table className="w-full min-w-[34rem] text-left text-sm"><thead className="bg-surface-muted text-xs uppercase tracking-[0.1em] text-muted"><tr><th className="px-4 py-3">Rank</th><th className="px-4 py-3">Entry</th><th className="px-4 py-3">Delegation</th><th className="px-4 py-3 text-right">Points</th></tr></thead><tbody className="divide-y divide-border">{placement.items.map((item) => <tr key={item.id}><td className="px-4 py-3 font-condensed font-bold">{item.rank}</td><td className="px-4 py-3 font-semibold">{item.entry || 'Entry'}</td><td className="px-4 py-3 text-muted">{item.delegation || 'Delegation'}</td><td className="px-4 py-3 text-right font-condensed font-bold">{item.points}</td></tr>)}</tbody></table></div>
...
export default function Index({ event, result_submissions: submissions = [], division_placements: placements = [], scope = {}, workspace = null }) {
    ...
    return (
        <AuthenticatedLayout header={<div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">{scopeLabel || event.name}</p><h1 className="text-2xl font-bold">Results</h1></div>}>
            ...
            <main className={adminStyles.page}><ResultsWorkspace event={event} workspace={workspace} scope={scope}><div className="mx-auto flex max-w-6xl flex-col gap-6">
                ...
                {flash?.status ? <div role="status" className="rounded-sm border border-primary bg-primary/10 px-4 py-3 text-sm font-semibold text-primary">{flash.status}</div> : null}
                {errors?.approval ? <div role="alert" className="rounded-sm border border-danger bg-danger-surface px-4 py-3 text-sm font-semibold text-danger">{errors.approval}</div> : null}
                ...
                {queue === 'needs' ? <>
                    ...
                    {submissions.length ? <section><div className="mb-3"><h2 className="text-2xl font-bold text-foreground">Match results</h2><p className="mt-1 text-sm text-muted">Confirm the score and winner, or return the submission with a correction reason.</p></div><div className="space-y-4">{submissions.map((submission) => <OutcomeReview key={submission.id} submission={submission}/>)}</div></section> : null}
                    {placements.length ? <section><div className="mb-3"><h2 className="text-2xl font-bold text-foreground">Final standings</h2><p className="mt-1 text-sm text-muted">This step awards the listed event points after the ranking is complete.</p></div><div className="space-y-4">{placements.map((placement) => <PlacementReview key={placement.id} placement={placement}/>)}</div></section> : null}
